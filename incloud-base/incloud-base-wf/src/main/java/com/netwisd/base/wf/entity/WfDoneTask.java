 ...
 */
@Data
@Table(value = "incloud_base_wf_done_task",comment = "已办任务")
@TableName("incloud_base_wf_done_task")
@EqualsAndHashCode(callSuper = true)
@ApiModel(value = "已办任务 Entity")
public class WfDoneTask extends IModel<WfDoneTask> implements WfTask{
    ...
    @Column(type = DataType.INT, length = 1,  isNull = false, comment = "流程决策标识")
    ...
    @Column(type = DataType.INT, length = 1,  isNull = false, comment = "是否同意")
    ...
    @Column(type = DataType.VARCHAR, length = 255,  isNull = false, comment = "处理意见")
     ...
     * 任务办理人名称
     */
    @ApiModelProperty(value="已办人名称")
    @TableField(value="assignee_name")
    @Column(type = DataType.VARCHAR, length = 50,  isNull = true, comment = "已办人名称")
    ...
    @Column(type = DataType.DATETIME, length = 0,  isNull = true, comment = "开始时间")
