...
public class GlobalExceptionHandlerAdvice {
    ...
    @ExceptionHandler({IncloudHttpException.class})
    @ResponseStatus(HttpStatus.INTERNAL_SERVER_ERROR)
    public Result httpException(IncloudHttpException exception) {
        log.error("IncloudHttpException:{}", exception);
        Result result = JacksonUtil.parseObject(exception.getMessage(), Result.class);
        return result;
    }
    ...
    @ResponseStatus(HttpStatus.INTERNAL_SERVER_ERROR)
    public Result processEngineException(ProcessEngineException exception) {
        String msg = exception.getMessage();
        msg = StrUtil.subBetween(msg,"Message: 'Exception while invoking TaskListener: ","'");
        log.error("processEngineException is:{}", msg);
