 ...
 */
@Data
@Table(value = "incloud_base_portal_content_adj_pictures",comment = "调整 图片轮播类内容发布")
@TableName("incloud_base_portal_content_adj_pictures")
@EqualsAndHashCode(callSuper = true)
@ApiModel(value = "调整 图片轮播类内容发布 Entity")
public class PortalContentAdjPictures extends WfEntity<PortalContentAdjPictures> {

    /**
     * link_main_pictures_id
     * 主表id
     */
    @ApiModelProperty(value="主表id")
    @TableField(value="link_main_pictures_id")
    @Column(type = DataType.BIGINT, length = 20,  isNull = false, comment = "主表id")
    private Long linkMainPicturesId;
    /**
     * portal_id
     * 所属门户ID
     */
